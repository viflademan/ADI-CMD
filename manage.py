...
def install(): #install zips
	...
	for file in toInstall: #loop for every asset in toInstall
		...
		for member in zFile.namelist(): #for every member in zipfile
			...
			if member[:8] == "Content/": #chop off Content/ from member location
				localDir = member[8:]
			elif member[:11] == "My Library/": #chop off My Library/ from member location
				localDir = member[11:]
			else:
				localDir = member
			if not filename:
				if not os.path.exists(path):
					os.makedirs(path)
				continue
			#doesn't work lol not fixing now
			#if os.path.exists(path):
			#	overwrite += 1
	
			# copy file (taken from zipfile's extract)
			source = zFile.open(member)
			dest = os.path.join(str(cfg.dazDir), localDir)
			pathFolder = dest.replace(filename, "")
